""" general functions for iP-FedLoRA """
...
def logits_softmax(logits):
    exp_logits = np.exp(logits - np.max(logits, axis=1, keepdims=True))  
    sums = np.sum(exp_logits, axis=1, keepdims=True)
    return exp_logits / sums

def weight_softmax(numbers):
    exp_values = [math.exp(x) for x in numbers]
    total = sum(exp_values)
    return [value / total for value in exp_values]

def pearson_correlation(probabilities1, probabilities2):
    mean1 = np.mean(probabilities1, axis=0)
    mean2 = np.mean(probabilities2, axis=0)
    numerator = np.sum((probabilities1 - mean1) * (probabilities2 - mean2), axis=0)
    denominator = np.sqrt(np.sum((probabilities1 - mean1) ** 2, axis=0) * np.sum((probabilities2 - mean2) ** 2, axis=0))
    return np.mean(numerator / denominator)  

def normalize_weights(sample_counts):
    total = sum(sample_counts)  
    if total == 0:
        return [0] * len(sample_counts)  
...
def setup_imports():
    ...
    files = []
    for package_name in ["trainers", "data", "models", "utils", "configs", "run"]:
