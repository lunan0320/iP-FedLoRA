...
def logit_payload_class_counts(payload):
    if not payload:
        raise ValueError("logit payload must not be empty")
    num_classes = payload[0].shape[-1]
    counts = torch.zeros(num_classes, dtype=torch.long)
    for batch_logits in payload:
        predictions = batch_logits.argmax(dim=1).cpu()
...
def filter_diverse_logit_payloads(payloads):
    class_counts = [
        logit_payload_class_counts(payload) for payload in payloads
    ]
    retained = [
        payload
        for payload, counts in zip(payloads, class_counts)
        if sum(int(count > 0) for count in counts) >= 2
    ]
    return retained, class_counts
...
class BaseSyncServerHandler(ParameterServerBackendHandler, ABC):  
    ...
    def logits_aggregation(self,payload):
        ...
        if len(self.client_buffer_cache) == self.cluster_num:
            retained_payloads, class_counts = (
                filter_diverse_logit_payloads(self.client_buffer_cache)
            )
            self.logger.info(
                "DIAGNOSTIC_TEACHER_FILTER "
                + json.dumps(
                    {
                        "received_teachers": len(self.client_buffer_cache),
                        "retained_teachers": len(retained_payloads),
                        "teacher_prediction_class_counts": class_counts,
                    },
                    sort_keys=True,
                )
            )
            if retained_payloads:
                self.Alogits,weights = self.calculate_pearson_correlation(
                    retained_payloads
                )
            else:
                self.Alogits = [
                    torch.zeros_like(batch_logits)
                    for batch_logits in self.client_buffer_cache[0]
                ]
                weights = []
